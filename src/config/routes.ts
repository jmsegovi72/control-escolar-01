export const ROUTES = {
  LOGIN: '/login',
  DASHBOARD: '/dashboard',
  CHANGE_PASSWORD: '/change-password',

  USERS: '/users',
  USERS_CREATE: '/users/create',
  USERS_SEARCH: '/users/search',
  USERS_DETAIL: '/users/detail',
  USERS_EDIT: '/users/edit',
  USERS_TOGGLE: '/users/toggle',
  USERS_UNLOCK: '/users/unlock',
  USERS_RESET_LOGIN: '/users/reset-login',

  PERSONS: '/persons',
  PERSONS_SEARCH: '/persons/search',
  PERSONS_CREATE: '/persons/create',
  PERSONS_DETAIL: '/persons/detail',
  PERSONS_EDIT: '/persons/edit',
  PERSONS_ADDRESSES: '/persons/addresses',
  PERSONS_DOCUMENTS: '/persons/documents',
  PERSONS_DOCUMENTS_UPLOAD: '/persons/documents/upload',
  PERSONS_DOCUMENTS_BULK_UPLOAD: '/persons/documents/bulk-upload',
  PERSONS_DOCUMENTS_LIST: '/persons/documents/list',
  PERSONS_DOCUMENTS_DETAIL: '/persons/documents/detail',
  PERSONS_DOCUMENTS_FILE: '/persons/documents/file',
  PERSONS_DOCUMENTS_EDIT: '/persons/documents/edit',
  PERSONS_DEMOGRAPHICS: '/persons/demographics',
  PERSONS_EMERGENCY: '/persons/emergency-contacts',
  PERSONS_BULK_LOAD: '/persons/bulk-load',

  ADDRESSES: '/persons/addresses',
  ADDRESSES_SEARCH: '/persons/addresses/search',
  ADDRESSES_CREATE: '/persons/addresses/create',
  ADDRESSES_DETAIL: '/persons/addresses/detail',
  ADDRESSES_EDIT: '/persons/addresses/edit',
  ADDRESSES_BULK_LOAD: '/persons/addresses/bulk-load',

  DEMOGRAPHICS: '/persons/demographics',
  DEMOGRAPHICS_SEARCH: '/persons/demographics/search',
  DEMOGRAPHICS_CREATE: '/persons/demographics/create',
  DEMOGRAPHICS_DETAIL: '/persons/demographics/detail',
  DEMOGRAPHICS_EDIT: '/persons/demographics/edit',
  DEMOGRAPHICS_BULK_LOAD: '/persons/demographics/bulk-load',

  EMERGENCY_CONTACTS: '/persons/emergency-contacts',
  EMERGENCY_CONTACTS_CREATE: '/persons/emergency-contacts/create',
  EMERGENCY_CONTACTS_DETAIL: '/persons/emergency-contacts/detail',
  EMERGENCY_CONTACTS_EDIT: '/persons/emergency-contacts/edit',
  EMERGENCY_CONTACTS_BULK_LOAD: '/persons/emergency-contacts/bulk-load',

  STUDENTS: '/students',
  STUDENTS_SEARCH: '/students/search',
  STUDENTS_CREATE: '/students/create',
  STUDENTS_DETAIL: '/students/detail',
  STUDENTS_EDIT: '/students/edit',
  STUDENTS_BULK_LOAD: '/students/bulk-load',
  STUDENTS_BATCH_EMAILS: '/students/batch-emails',
  STUDENTS_BATCH_CODES: '/students/batch-codes',
  STUDENTS_DOCUMENTS: '/students/documents',
  STUDENTS_DOCUMENTS_UPLOAD: '/students/documents/upload',
  STUDENTS_DOCUMENTS_LIST: '/students/documents/list',
  STUDENTS_DOCUMENTS_DETAIL: '/students/documents/detail',
  STUDENTS_DOCUMENTS_FILE: '/students/documents/file',
  STUDENTS_DOCUMENTS_EDIT: '/students/documents/edit',
  STUDENTS_ACADEMIC_BACKGROUND: '/students/academic-background',
  STUDENTS_ADMISSION: '/students/admission',
  STUDENTS_ENROLLMENT: '/students/enrollment',
  STUDENTS_GRADES: '/students/grades',

  CATALOGS_ZIP_CODES: '/zip-codes',
  CATALOGS_ZIP_CODES_CREATE: '/zip-codes/create',
  CATALOGS_CLASSES: '/classes',
} as const;
