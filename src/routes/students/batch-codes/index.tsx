...
const CODE_NUMBER_REGEX = /^[A-Z]{2}\d{7,8}$/i;

const m = messages.students.batchCodes;
...
export default component$(() => {
  ...
  const processFile = $(async (file: File) => {
    ...
    reader.onload = $((e: ProgressEvent<FileReader>) => {
      ...
      const parsedRows: ImportRow[] = rawRows.map((rawRow, idx) => {
        ...
        // 2. Validación de matrícula (codeNumber)
        if (!codeNumberVal) {
          obs.push('Formato de matrícula inválido');
          status = 'invalid';
        } else if (!CODE_NUMBER_REGEX.test(codeNumberVal)) {
          obs.push('Formato de matrícula inválido');
          status = 'invalid';
        }
  ...
  const downloadTemplate$ = $(() => {
    const headers = ['id', 'Matrícula'];
    const exampleRow = ['1', 'SC2024001'];
    ...
    link.href = url;
    link.setAttribute('download', 'plantilla_matriculas.csv');
  ...
  const handleImport$ = $(async () => {
    ...
    try {
      ...
      await studentService.updateCodesBatch({ updatesCodes: updatesDto });
      ...
      if (errorResponse?.status === 404) {
        try {
          ...
          const normalized = normalizeError(
            fallbackErr,
            'Error al actualizar matrículas de forma individual.',
          );
      ...
      const normalized = normalizeError(
        err,
        messages.errors.updateCodesBatchFailed ||
          'Error al actualizar las matrículas.',
      );
      errorMsg.value = normalized.message;
      step.value = 'error';
    } finally {
            ...
            <div class="bulk-load-page__card">
              ...
              >
                ...
                <div class="import-upload-info">
                  ...
                  >
                    Guía de Campos del Archivo CSV de Matrículas:
                  </span>
                  <ul
                    style={{
                      margin: 0,
                  ...
                  >
                    <li>
                      ...
                      previamente), Matrícula (Comenzar con 2 letras y tener de
                      7 a 8 números, ej: SC2024001).
                    </li>
                    <li
                      style={{
                        listStyleType: 'none',
          ...
          {!validating.value && step.value === 'review' && (
            <div class="bulk-load-page__review">
              ...
              <Panel
                ...
                description="Selecciona las matrículas válidas que deseas actualizar en el sistema."
              >
                <DataTable
                  columns={columns}
                  rows={tableRows}
            ...
            <div class="bulk-load-page__card">
              <Panel>
                <div class="import-success-panel">
                  ...
                  <p>
                    Se han actualizado exitosamente las matrículas de{' '}
